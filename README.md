...
| | 其他飞书 MCP 方案 | lark-mcp-bridge |
|---|---|---|
| **认证** | 需创建飞书开放平台应用 + 配置 App ID/Secret + 手动授权 scope | `lark-cli auth login` 一次登录即用 |
| **API 覆盖** | 10-20 个手工封装的固定工具 | 19 个内置 + 219 个可发现的 API |
| **扩展性** | 新功能需等作者更新 | Agent 通过 `lark.discover` 自主探索 |
| **安全** | 无或简单 | 白名单 + 风险分级 + 审计日志 |
| **用户门槛** | 需开发者背景 | 普通用户：安装 → 登录 → 用 |
